...
export function MenuEntrada({ onSesion }: PropsMenuEntrada): JSX.Element {
  ...
  async function entrar(canalId: string, nombre: string, cupo: number | null): Promise<void> {
    setOcupado(true);
    setError(null);
    try {
      // crearCanal es idempotente en el backend: si el canal ya existe lo
      // devuelve tal cual sin tocar su cupo. Por eso sirve tanto para crear
      // uno nuevo como para "entrar" a uno existente sin logica extra aqui.
      await api.crearCanal(canalId, cupo);
